...
$sql = "SELECT title, snippet, news.url, image.url as image, source.short_name as source FROM news INNER JOIN image ON news.image_id = image.id INNER JOIN source ON news.source_id = source.id WHERE news.image_id > 0 AND snippet <> 'NULL' AND news.datetime_created > '" . $day_minus_1d . "' ORDER BY news.datetime_created DESC LIMIT " . $MAX_RECORD;
